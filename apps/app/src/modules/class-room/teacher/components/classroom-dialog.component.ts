...
export interface ClassroomDialogModel {
  name: string;
  description?: string;
  enabled?: boolean;
  configuration?: {
    ...
    <div *ngIf="open" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="relative bg-white rounded-2xl shadow-xl w-full max-w-lg mx-4 overflow-hidden">
        <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
          <h3 class="text-lg font-semibold text-gray-900">{{ title || 'Classroom' }}</h3>
        ...
        <div class="px-5 py-4 space-y-4">
          <div>
            ...
            <input [(ngModel)]="model.name" type="text" class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-gray-900 focus:border-gray-900" placeholder="Enter classroom name" />
          </div>
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-3">Enabled Technologies</label>
            <div class="space-y-2">
              <label class="inline-flex items-center space-x-2 select-none">
                <input [(ngModel)]="model.configuration!.enabledTechnologies.html" type="checkbox" class="rounded border-gray-300 text-gray-900 focus:ring-gray-900" />
                <span class="text-sm text-gray-700">HTML</span>
              </label>
              <label class="inline-flex items-center space-x-2 select-none">
                <input [(ngModel)]="model.configuration!.enabledTechnologies.css" type="checkbox" class="rounded border-gray-300 text-gray-900 focus:ring-gray-900" />
                <span class="text-sm text-gray-700">CSS</span>
              </label>
              <label class="inline-flex items-center space-x-2 select-none">
                <input [(ngModel)]="model.configuration!.enabledTechnologies.javascript" type="checkbox" class="rounded border-gray-300 text-gray-900 focus:ring-gray-900" />
...
export class ClassroomDialogComponent {
  ...
  @Input() model: ClassroomDialogModel = {
    name: '',
    enabled: true,
    configuration: {
      enabledTechnologies: {
  ...
  @Output() save = new EventEmitter<ClassroomDialogModel>();
  ...
  onSave() {
    ...
    this.save.emit({
      ...this.model,
      configuration: {
        enabledTechnologies: { ...this.model.configuration!.enabledTechnologies }
      }
    });
  }
}